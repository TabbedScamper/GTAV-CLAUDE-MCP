...
def handle_inspect(params: dict) -> dict:
    ...
    if struct_hint == "wheel":
        ...
        # parser-dump struct (e.g. "CHandlingData") -> auto-label every known field offset.
        # Needs re_tools_pardump loaded with a dump for this build (pardump.load_index(...)).
        try:
            import re_tools_pardump as _pd
            _info = _pd.struct_fields(struct_hint)
            if _info.get("success"):
                for _f in _info["fields"]:
                    labels[int(_f["offset"], 16)] = _f["name"]
        except Exception:
            pass
...
def _aob_to_regex(pat, mask):
    """Compile a parsed AOB (byte list + wildcard mask) to a regex for C-speed scanning.
    Literal bytes are escaped, wildcards become '.', and re.DOTALL makes '.' also match 0x0A.
    Mirrors the masked-pattern approach in pymem/GuidedHacking (re.finditer over each region)."""
    parts = [re.escape(bytes([b])) if m else b"." for b, m in zip(pat, mask)]
...
def handle_scan_pattern(params: dict) -> dict:
    ...
    plen = len(pat)
    rx = _aob_to_regex(pat, mask)   # masked AOB -> regex; match at C speed (vs the old per-byte loop)
    end = base + size
    ...
    def poll():
        ...
        if data:
            for m in rx.finditer(data):           # C-speed scan (pymem/GuidedHacking approach)
                a = cur + m.start()
...
try:
    import re_tools_scan as _rts
    _rts.bind(globals())
    COMMANDS.update(_rts.SCAN_COMMANDS)
    _OFFTHREAD_COMMANDS |= _rts.SCAN_OFFTHREAD            # read-only -> paused-safe
    log_message("info", "re_tools_scan (value scan) loaded")
except Exception as e:
    log_message("error", f"re_tools_scan load failed: {e}")
try:
    import re_tools_patch as _rtpatch
    _rtpatch.bind(globals())
    COMMANDS.update(_rtpatch.PATCH_COMMANDS)
    _OFFTHREAD_COMMANDS |= _rtpatch.PATCH_OFFTHREAD       # only the read-only ones are paused-safe
